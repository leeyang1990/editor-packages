...
export class Matrix {
	...
	public concat(target: Matrix): void {
		let a = this.a * target.a;
		let b = 0.0;
		let c = 0.0;
		let d = this.d * target.d;
		let tx = this.tx * target.a + target.tx;
		let ty = this.ty * target.d + target.ty;
	...
	public $preConcat(target: Matrix): void {
		let a = target.a * this.a;
		let b = 0.0;
		let c = 0.0;
		let d = target.d * this.d;
		let tx = target.tx * this.a + this.tx;
		let ty = target.ty * this.d + this.ty;
	...
	public rotate(angle: number): void {
		...
		if (angle !== 0) {
			...
			const u = NumberUtils.cos(angle);
			const v = NumberUtils.sin(angle);
			const ta = this.a;
			const tb = this.b;
			const tc = this.c;
			const td = this.d;
	...
	public deltaTransformPoint(pointX: number, pointY: number, resultPoint?: Point): Point {
		const x = this.a * pointX + this.c * pointY;
		const y = this.b * pointX + this.d * pointY;
		if (resultPoint) {
			resultPoint.setTo(x, y);
			return resultPoint;
		}
	...
	public createBox(scaleX: number, scaleY: number, rotation?: number, tx?: number, ty?: number): void {
		...
		if (rotation !== 0) {
			...
			const u = NumberUtils.cos(rotation);
			const v = NumberUtils.sin(rotation);
			self.a = u * scaleX;
			self.b = v * scaleY;
			self.c = -v * scaleX;
			self.d = u * scaleY;
